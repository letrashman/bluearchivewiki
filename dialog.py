...
def generate():
    ...
    for character in data.characters.values():
        ...
        def get_dialog_lines(dialog_data):
            missing_tl = []
        ...
        lines, missing_tl['DataList'] = get_dialog_lines(data.character_dialog)
        ...
        with open(os.path.join(args['outdir'], f'{character.name_translated}_dialog.txt'), 'w', encoding="utf8") as f:
            wikitext = template.render(lines=lines,character=character)
            f.write(wikitext)
...
def save_missing_translations(name, data):
    ...
    f = open(args['translation'] + '/' + name + '.json', "w", encoding='utf8' )
